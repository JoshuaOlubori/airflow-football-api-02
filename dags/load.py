"""DAG that loads fixtures and weather data from MinIO to DuckDB."""
...
@dag(
    ...
    # this DAG runs as soon as the fixture data is ready in MinIO
    schedule=[gv.DS_FIXTURES_DATA_MINIO,],
    catchup=False,
    default_args=gv.default_args,
    description="Loads fixtures from MinIO to DuckDB.",
...
def load_data():
    ...
    @task
    def get_copy_args(obj_list_fixtures):
        """Return tuples with bucket names and bucket contents."""

        return [
            {
                "source_bucket_name": gv.FIXTURES_BUCKET_NAME,
                "source_object_names": obj_list_fixtures,
                "dest_object_names": obj_list_fixtures,
            },
        ]
    ...
    copy_objects_to_archive = MinIOCopyObjectOperator.partial(
        ...
        get_copy_args(
        list_files_fixtures_bucket.output
        )
    )
